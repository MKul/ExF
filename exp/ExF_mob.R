...
threshold = exp(-5)     #level at which edge are deleted
bins = seq(0,1,0.01)
#bins = c(0,0.5,1)
...
loadData <- function(dataset){
  ...
  if(dataset=="mobility"){
    data = read.csv2("e:/_Politechnika_Wroclawska/Projekty/ExF/mobility.csv")
    data = data[,]
    return(data)
  }
}
...
data = loadData(dataset)
tn = findAllNodes(data)
threshold = exp(-5)  #5 min
results1 <- runEvents(tr = threshold, norm = norm, unit = unit, symdt = symdt, show = show, bins = bins)
threshold = exp(-15) #15 min
results2 <- runEvents(tr = threshold, norm = norm, unit = unit, symdt = symdt, show = show, bins = bins)
threshold = exp(-30) #30 min
results3 <- runEvents(tr = threshold, norm = norm, unit = unit, symdt = symdt, show = show, bins = bins)
threshold = exp(-60) #60 min
results4 <- runEvents(tr = threshold, norm = norm, unit = unit, symdt = symdt, show = show, bins = bins)
